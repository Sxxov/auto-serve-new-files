export class TimeUtility {
    static hhmmss(ms, isSecondsFloored = true) {
        ...
        return `${String(hrs).padStart(2, '0')}:${String(mins).padStart(2, '0')}:${String(secs).padStart(2, '0')}${isSecondsFloored
            ? ''
            : `.${String(msOnly).padStart(2, '0').replace('.', '')}`}`;
    }
    static msStrict(hhmmss) {
    ...
    static ms(hhmmss) {
        ...
        if (hhmmssParts.length < 3) {
            hhmmssParts.unshift(...new Array(3 - hhmmssParts.length).fill(0));
        }
