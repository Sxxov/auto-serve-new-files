...
export class WalkUtility {
    ...
    static walkAndReturnVoid(object, callback, key, parent) {
        if (typeof object !== 'object') {
        ...
        for (let i = 0, l = keys.length; i < l; ++i) {
            const key = keys[i];
            const value = object[key];
            this.walkAndReturnVoid(value, callback, key, object);
        }
    }
    ...
    static walkAndAssertShape(obj1, obj2, path = '.') {
        if (obj2 instanceof EitherShape) {
            const shape = obj2.shapes.find((shape) => shape !== null && typeof shape === 'object' && shape.constructor
                ? obj1 instanceof shape.constructor
                : typeof obj1 === typeof shape);
            if (shape == null && !obj2.shapes.includes(shape)) {
                throw new AssertionError(`Value in "${path}" is not one of shape "${obj2.shapes
